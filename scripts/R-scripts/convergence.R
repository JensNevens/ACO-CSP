do.lines.plot <- function(data.list=NULL, test.names=names(data.list), plot.mar=c(6,16,4,3), plot.title="Sample acotsp", plot.yaxis.label="Mean solution quality",plot.xaxis.label="Evaluations", output="sample-acotsp.png", plot.axis.style=1, plot.size.axis=3, plot.size.title=3){
  
  # Get x&y axis bounds
  x.min <- min(unlist(lapply(data.list,function(data) return(min(data$budget)))))
  x.max <- max(unlist(lapply(data.list,function(data) return(max(data$budget)))))
  ...
  plot(data.list[[1]]$budget, data.list[[1]]$quality, ylab=NA, type="n", main=plot.title, xlim=c(x.min,x.max),ylim=c(y.min, y.max), xlab=plot.xaxis.label)

  for(i in 1:length(data.list)){
     name <- test.names[i]
     # Complete lines for a nicer visualization
     data.x <- c(x.min, data.list[[name]]$budget, x.max)
